# Phylogenetically informed fossil prediction
...
To calculate the phylogenetic adjustment, we solve $V_{lam} w = e$, giving covariance-weighted residuals $w = V_{inv}e$ of dimensions $n \times 1$. This is inverse-covariance weighting; it does not by itself produce unit-variance independent residuals. Multiplication by $V_{cross}^T$ projects these weighted residuals onto the fossil occurrences. The correction depends jointly on their covariances with all training taxa and on the training residuals, yielding an $m \times 1$ adjustment vector.

The two components are then added together to yield $\hat{y} = X\beta + V_{cross}^T V_{inv} e$, an $m \times 1$ vector of predictions. For MAP, log predictions are averaged within each site and then exponentiated, giving the geometric mean of occurrence predictions. Validation scores the same mean log prediction. The retained LMA workflow back-transforms before arithmetic site averaging.

This framework is applied to three targets: MAT (°C), log(MAP) (log cm), and log₁₀(LMA) (log₁₀ g m⁻²). For MAT and MAP, twelve fossil-measurable leaf traits serve as predictors and separate PGLS models are fitted for each target; the MAP model exponentiates the site-mean log prediction, while the retained LMA workflow applies $10^{\hat{y}}$ before site averaging. For LMA, the single predictor is $\log_{10}(\text{PW}^2/A)$ (petiole width squared divided by blade area), following Dana Royer (pers. comm.). This PGLS implementation uses one row per extant operational taxon and fits grand means across training sites; that is a modelling choice, not a universal requirement of phylogenetic methods. This averages away within-species cross-site variation during fitting; held-out and fossil prediction rows instead use within-site means. For the climate models, fossil occurrences are placed separately at their reported site ages, using formal genus, family, and order names in the primary analysis. Quoted ranks are excluded from primary placement and included only in a sensitivity analysis. Consequently, occurrences of the same reported species can receive different phylogenetic adjustments when their placement ages differ.
...
Multiply the inverse covariance matrix by the modern residuals to obtain covariance-weighted residuals.
$$V_{inv} e = \begin{bmatrix} 1.19 & -0.48 \\ -0.48 & 1.19 \end{bmatrix} \begin{bmatrix} -3 \\ -2 \end{bmatrix} = \begin{bmatrix} -2.61 \\ -0.94 \end{bmatrix}$$

Multiply the cross-covariance matrix by the weighted residuals to project the adjustment onto the fossil species.
