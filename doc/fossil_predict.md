...
The two components are then added together for each species-by-site row to yield $\hat{y} = X\beta + V_{cross}^T V_{inv} e$. This produces an $m \times 1$ vector of per-(species, site) predictions, which are averaged within each fossil site to obtain site-level MAT and MAP estimates. Because the precipitation model operates on $\log(\text{MAP})$, $\exp()$ is applied before site-averaging to return values to a linear scale.
