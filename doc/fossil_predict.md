...
This framework is applied to three targets: MAT (°C), log(MAP) (log cm), and log₁₀(LMA) (log₁₀ g m⁻²). For MAT and MAP, twelve fossil-measurable leaf traits serve as predictors and separate PGLS models are fitted for each target; the MAP model applies $\exp()$ and the LMA model applies $10^{\hat{y}}$ before site-averaging to return predictions to a linear scale. For LMA, the single predictor is $\log_{10}(\text{PW}^2/A)$ (petiole width squared divided by blade area), following Dana Royer (pers. comm.) who established that LMA scales linearly with this ratio in log–log space. Although $\beta$, $V_{inv}$, and $e$ are estimated from extant species-level means, the fossil prediction unit is a species × site occurrence. $X$ is built from within-site species means, and each occurrence is grafted independently at its own site age to obtain its own column of $V_{cross}$ and hence its own phylogenetic adjustment. This rule also applies when genus, family, or order is the finest available identification. No fossil physiognomic traits or ages are averaged across sites in the primary climate or LMA analyses; occurrence predictions are averaged only afterward to obtain site-level estimates.
