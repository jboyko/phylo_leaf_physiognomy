...
# Regenerates the pre-baked model objects and constants used by dilp_pgls().
# Outputs land in dilp_update/; the printed steps say where to copy them.
...
# 2. Regression coefficients — named numeric vectors from the impute PGLS fit,
#    so X %*% pgls_beta_* works directly inside dilp_pgls().
...
# 3. Phylogenetic weight vectors Ke = solve(V_lam) %*% resid, pre-multiplied so
#    prediction only needs t(V_cross) %*% Ke. Named by training species.
...
# 4. Scaffold tree — copy to inst/extdata/ whenever the training species list
#    or the WCVP tree changes.

file.copy("data/tre_scaffold.tre", "dilp_update/tre_scaffold.tre", overwrite = TRUE)
cat("Copied tre_scaffold.tre\n")

# 5. RMSE constants for the uncertainty columns, taken from the CV results so
#    the literals in dilp_pgls.R stay in sync with the pipeline.
...
# 6. Patch sysdata.rda — overwrite only the four PIP objects, preserving the
#    rest, and write the result to dilp_update/sysdata.rda.

# Path to a local checkout of the dilp package source; set DILP_SYSDATA in the
# environment to point elsewhere. Only needed to rebuild the package objects.
