...
# ----- Including phylogenetic structure -----
# -----------------------------------------------
addFossil <- function( Vmat, lineage, date, newname) {
	ht <- max(Vmat)
	idx <- which(row.names( V ) == lineage)
	fossil <- V[idx, ]
	i <- which(fossil > ( ht- date) )
	fossil[i] <-  ( ht - date)
	Vmat <- rbind( Vmat, fossil )
	Vmat <- cbind( Vmat, c( fossil, ht - date)  )
	rownames(Vmat)[length(fossil) + 1] <- newname
	colnames(Vmat)[length(fossil) + 1] <- newname
	return(Vmat)
	}
