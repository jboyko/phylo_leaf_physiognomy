...
# This version allows for self-validation (predicting each taxon included in the analysis)
...
# -----------------------------------------------
# Modifications: 16/6/16
# Create loop to run pglm on full data, but introduce one missing value at the weighting stage.
# Repeat for all species to generate predictions for each species, while using full dataset to generate matrices.
...
# loop over species
	all_stuff<-vector("list",length=dim(data)[1])	#create vector to hold results
	
	for(i in 1:dim(data)[1] )	{
		suppressWarnings(rm(iy,missY,idx.miss,res,coefs,W,preds))	#tidy up old variables, but supress warnings that occur when first run and variables don't exist
		
		
		data2 <- data
		data2[i,2]<-NA		#remove data for one species

		iy <- which( colnames(data2) == depVar )
		
		missY <- is.na(data2[,iy] )
		
		names(missY) <- rownames(data2)
		
		idx.miss <- which( missY == TRUE )
		
		
		res <- residuals(model)
		res <- res[-i,]	#remove residual value for species that has been removed
		
		X1 <- X[idx.miss, ]	#use temporary X variable to allow reuse
		...
		all_stuff[[i]] <- list(preds = preds, vars = vars, modsum = model, resids = res, LL = model$logLikY, 
...
# -----------------------------------------------
# ---- Predict missing data with lambda-----
# -----------------------------------------------
predictPGLMestLambda <- function( formula, data, phylomat, lambda = 'ML') {
	lamTrans <- function(Vmat, lambda) {
		V1 <- Vmat
		diag(V1) <- 0
		V2 <- diag( diag(Vmat), ncol = length(Vmat[1,]), nrow = length(Vmat[,1]))
...
# ----- Including phylogenetic structure -----
# -----------------------------------------------
weights.p <- function( Vmat, miss) {
	Vmat <- order.V( Vmat )
	miss <- sortVec( miss )
	Vmiss <- matrix( Vmat[miss == TRUE,] , ncol = dim(Vmat)[1])
